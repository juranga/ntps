...
from UI.Components.TopLevelControllers.LivePacketComponents.Queue_Size_Component import Queue_Size_Component
...
class Live_Packet_Controller():
    ...
    def __init__(self, proxy_server = Proxy_Server()):
        ...
        self.drop_button = Drop_Button(proxy_server, self.field_area)
        self.forward_button = Forward_Button(proxy_server, self.field_area)
        self.queue_edit = Queue_Size_Component(proxy_server)
        
        self.view =  Live_Packet_View(self.proxy_toggle, self.intercept_toggle,
                                      self.packet_area, self.packet_binary_area,
                                      self.packet_hex_area, self.field_area,
                                      self.forward_button, self.save_button,
                                      self.drop_button, self.queue_edit)
